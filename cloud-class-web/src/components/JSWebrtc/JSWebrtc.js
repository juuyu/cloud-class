let JSWebrtc = {
    ...
    ParseUrl: function (rtmp_url) {
        ...
        a.href = rtmp_url.replace("rtmp://", "https://").replace("webrtc://", "https://").replace("rtc://", "https://");
...
JSWebrtc.Player = function () {
    ...
    Player.prototype.update = function () {
        ...
        if (!this.isPlaying) {
            this.isPlaying = true;
            this.options.video.play().then(res => {
                if (this.options.thenCb) {
                    this.options.thenCb(res)
                }
            }).catch(
                err => {
                    if (this.options.errCb) {
                        this.options.errCb(err)
                    }
